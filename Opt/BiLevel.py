...
# Parameters
model.upper_bound = Param(modes, within=NonNegativeReals, 
    initialize={'Truck': 0.4, 'Train': 0.3, 'Ship': 0.3})
model.lower_bound = Param(routes, within=NonNegativeReals, 
    initialize={'Route1': 0.2, 'Route2': 0.2, 'Route3': 0.6})
...
model.obj = Objective(expr=sum(model.x[mode] for mode in modes) 
    + sum(model.y[mode, route] for mode in modes for route in routes), sense=minimize)
